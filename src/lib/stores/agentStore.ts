...
interface AgentState {
  ...
  sendMessage: (
    sessionId: string,
    message: Omit<
      TextChatMessage | FileChatMessage,
...
export const useAgentStore = create<AgentState>((set, get) => ({
    ...
    try {
      ...
      if (data) {
        set({
          ...
          isLoading: false,
          activeSessionId:
            data.sessions.find((s: ChatSession) => s.status === "active")?.id ||
            data.sessions.find((s: ChatSession) => s.status === "pending")
              ?.id ||
            null,
        });
      } else {
        set({
          ...
          isLoading: false,
          activeSessionId: null,
          error: json && json.error ? json.error : "Failed to fetch agent data",
        });
      }
    } catch (e) {
      set({
  ...
  setActiveSessionId: (sessionId) => {
    const customerId = get().sessions.find(
      (s) => s.id === sessionId
    )?.customerId;
    if (customerId) {
      const isBlocked = get().settings?.blockedIps.includes(
        get().customers.find((c) => c.id === customerId)?.ipAddress || ""
      );
      if (isBlocked) {
        ...
        sessions: state.sessions.map((session) => ({
          ...
          messages: session.messages.map((message) =>
            message.agentId === agentId
              ? { ...message, agentId: updatedAgent.id }
              : message
          ),
        })),
      }));

      // 同时更新adminStore中的agents列表
      const { updateAgent } = useAdminStore.getState();
      if (updateAgent) {
  ...
  updateSettings: async (agentId, settings) => {
    const updatedSettings = await mockApi.updateAgentSettings(
      agentId,
      settings
    );
    if (updatedSettings) {
  ...
  pollSessionUpdates: async (sessionId: string) => {
    ...
    try {
      await _fetchSession(sessionId, set);
      logger.debug("Session updated via polling", { sessionId });
    } catch (error) {
      ErrorHandler.handle(error, "AgentStore.pollSessionUpdates");
    }
  },
  ...
  blockCustomer: (ipAddress) => {
    set((state) => {
      if (!state.settings || state.settings.blockedIps.includes(ipAddress))
        return {};
      const newSettings = {
        ...state.settings,
  ...
  deleteCustomer: (customerId) => {
    set((state) => {
      const sessionsToRemove = state.sessions
        .filter((s: ChatSession) => s.customerId === customerId)
        .map((s: ChatSession) => s.id);
  ...
  archiveSession: async (sessionId) => {
    const updatedSession = await mockApi.updateSessionStatus(
      sessionId,
      "archived"
    );
    if (updatedSession) {
      set((state) => {
        const otherSessions = state.sessions.filter(
          (s: ChatSession) => s.status !== "archived" && s.id !== sessionId
        );
        return {
  ...
  sendFileMessage: async (sessionId, file) => {
    ...
    const persistedMessage = (await mockApi.sendMessage(
      sessionId,
      tempFileMessage as FileChatMessage
    )) as FileChatMessage;
    ...
    set((state) => ({
      sessions: state.sessions.map((s) =>
        s.id === sessionId
          ? { ...s, messages: [...s.messages, persistedMessage] }
          : s
      ),
    }));
    ...
    const interval = setInterval(() => {
      set((state) => {
        ...
        const messageToUpdate = currentSession.messages.find(
          (m) => m.id === persistedMessage.id
            ...
            s.id === sessionId
              ? {
                  ...
                  messages: s.messages.map((m) =>
                    m.id === persistedMessage.id ? updatedMessage : m
                  ),
                }
              : s
          ),
