...
export function ChatWindow({ session, customer }: ChatWindowProps) {
  ...
  const handleRedact = async () => {
    ...
    try {
      ...
      ErrorHandler.handle(error, "ChatWindow.redactMessage");
      toast({
        title: "处理失败",
              ...
              >
                ...
                >
                  {msg.type === "text" ? (
                    ...
                    <a
                      href={(msg as FileChatMessage).file.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-3"
                    >
                      <CircularProgress
                        progress={(msg as FileChatMessage).file.progress}
                      />
                      <div>
                        <p className="text-sm font-medium break-all underline">
                          {(msg as FileChatMessage).file.name}
                        </p>
                        <p
                          className={cn(
                            "text-xs",
