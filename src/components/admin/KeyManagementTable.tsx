...
export function KeyManagementTable() {
  const { keys, fetchKeys, isLoading, createKey, updateKey, deleteKey } =
    useAdminStore();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingKey, setEditingKey] = useState<AccessKey | null>(null);
  const { toast } = useToast();
  const { start: startTimer, end: endTimer } = usePerformanceMonitor(
  ...
  const handleCopyKey = async (key: string) => {
    try {
      ...
      logger.logUserAction("copy_key", "admin", { keyLength: key.length });
    } catch (error) {
      ErrorHandler.handle(error, "KeyManagementTable.copyKey");
    }
  };
  ...
  const handleToggleStatus = async (key: AccessKey) => {
    ...
    try {
      ...
      toast({
        title: "状态已更新",
        description: `"${key.name}"的密钥已被${
          newStatus === "active" ? "激活" : "暂停"
        }。`,
      });

      logger.logUserAction("toggle_key_status", "admin", {
        keyId: key.id,
      ...
      ErrorHandler.handle(error, "KeyManagementTable.toggleStatus");
    }
  };
              ...
              keys.map((key) => (
                <TableRow key={key.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        key.key_type === "admin" ? "default" : "secondary"
                      }
                    >
                      {key.key_type}
                    </Badge>
                  ...
                  <TableCell>
                    {key.expiresAt
                      ? format(parseISO(key.expiresAt), "PPP HH:mm", {
                          locale: zhCN,
                        })
                      : "永不"}
                  </TableCell>
                  <TableCell>
                    {format(parseISO(key.createdAt), "PPP", { locale: zhCN })}
                  </TableCell>
                  <TableCell className="text-right">
                    <DropdownMenu>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {editingKey
              ? "更新此访问密钥的名称或备注。"
              : "将生成一个新密钥。若类型为 'agent'，该密钥在首次登录时会自动创建并绑定坐席。"}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            {!editingKey && (
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Select
                  onValueChange={(value: AccessKeyType) => setKeyType(value)}
