...
export function VisitorChat({ aliasToken }: VisitorChatProps) {
  ...
  const { start: startSendTimer, end: endSendTimer } = usePerformanceMonitor(
  ...
  useEffect(() => {
    if (data?.session?.messages) {
      const welcomeMessages = data.session.messages.filter(
        (m) => m.sender === "agent"
      );
      const otherMessages = data.session.messages.filter(
        (m) => m.sender !== "agent"
      );
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setData((d) =>
        d
          ? {
              ...
              session: {
                ...d.session,
                messages: [...d.session.messages, sentMessage],
              },
            }
          : null
      );
      ...
      logger.logUserAction("visitor_send_message", aliasToken || "unknown", {
        messageLength: message.length,
        duration,
      });
    } catch (e) {
      ErrorHandler.handle(e, "VisitorChat.sendMessage");
    } finally {
      setIsSending(false);
    }
  };
    ...
    <>
      <header className="p-4 border-b flex items-center gap-4 bg-muted/30 rounded-t-2xl">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground capitalize">
            {translateStatus(agent.status as "online" | "busy" | "offline")}
          </p>
        </div>
      </header>
                ...
                >
                  <p className="text-sm">
                    {msg.type === "text"
                      ? msg.text
                      : msg.type === "file"
                      ? (msg as import("@/lib/types").FileChatMessage).file.name
                      : "未知消息类型"}
                  </p>
                </div>
                {msg.sender === "customer" && (
      ...
      <footer className="p-4 border-t bg-muted/30 rounded-b-2xl">
        <div className="relative">
          <Label htmlFor="visitor-chat-message" className="sr-only">
            输入您的消息
          </Label>
          <Textarea
